...
PfFfMacScheduler::HarqProcessAvailability (uint16_t rnti)
{
  ...
  do
    {
      i = (i + 1) % HARQ_PROC_NUM;//HARQ_proc_num = 8(pre defined)
    }
  while ( ((*itStat).second.at (i) != 0)&&(i != (*it).second));//check this later 
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  m_rachAllocationMap.resize (m_cschedCellConfig.m_ulBandwidth, 0);//for which Rb which UE is allocated i.e.., mp[Rb_idx] = ue_rnti
  uint16_t rbStart = 0;
  ...
  for (int i = 0; i < rbgNum; i++)
    {
      ...
      if (rbgMap.at (i) == false)
        {
          std::map <uint16_t, pfsFlowPerf_t>::iterator it;
          std::map <uint16_t, pfsFlowPerf_t>::iterator itMax = m_flowStatsDl.end ();// m_flowStatsDl is UE statistics per RNTI
          double rcqiMax = 0.0;
...
//https://my.redrob.io/assess/65bcdc0938215bd9aca766d4
                      if (rcqi > rcqiMax)
                        {
                          rcqiMax = rcqi;
                          itMax = it;
